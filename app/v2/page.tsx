...
export default function DashboardV2() {
  ...
  const [isConnected, setIsConnected] = useState(false); // State baru buat koneksi
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (sensorData && sensorData.length > 0) {
        ...
        // LOGIKA HEARTBEAT: Jika data terakhir lebih lama dari 15 detik = Disconnect
  ...
  useEffect(() => {
    fetchData();
    const interval = setInterval(fetchData, 5000); // Cek tiap 5 detik
  ...
  return (
    <div className="w-full space-y-12">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
             <div className="relative flex h-3 w-3">
                {/* Animasi warna dinamis berdasarkan KONEKSI bukan sekedar pompa */}
             ...
             <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 flex items-center gap-2">
               ...
               <span className={isConnected ? 'text-emerald-600' : 'text-rose-600'}>
                 {isConnected ? 'LIVE CONNECTED' : 'DISCONNECTED (ESP32 OFFLINE)'}
               </span>
      ...
      {/* 3 KARTU UTAMA */}
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 text-white transition-opacity duration-500 ${isConnected ? 'opacity-100' : 'opacity-60 grayscale-[0.5]'}`}>
      ...
      <div className="flex flex-col xl:flex-row gap-8 items-start">
        ...
        <div className={`w-full xl:w-72 p-8 rounded-[2.5rem] border-2 transition-all duration-500 shadow-2xl ${
          ...
          : 'bg-rose-50 border-rose-100 shadow-rose-200/20 opacity-90'
        }`}>
          ...
          <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${sensor.kondisi_pompa === 'HIDUP' && isConnected ? 'text-emerald-600' : 'text-rose-400'}`}>
             Pump Analysis
          </p>
          ...
          <p className="text-[10px] mt-3 font-bold opacity-60 italic leading-tight">
            {!isConnected ? 'Waiting for ESP32...' : sensor.kondisi_pompa === 'HIDUP' ? 'Irrigating right now...' : 'Pump is on standby.'}
          </p>
        </div>
        ...
        <div className="flex-1 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-8 py-5 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
             ...
             {!isConnected && <span className="text-[9px] font-black text-rose-500 animate-pulse bg-rose-50 px-2 py-1 rounded-full uppercase">Connection Lost</span>}
          </div>
